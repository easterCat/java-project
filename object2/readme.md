...
把field从public改成private，外部代码不能访问这些field，那我们我们需要使用方法（method）来让外部代码可以间接修改field
...
有public方法，自然就有private方法。和private字段一样，private方法不允许外部调用，内部方法是可以调用private方法的。
...
calcBirthday()是一个private方法，外部代码无法调用，但是，内部方法setBirthday()可以调用它。
...
调用这个setHighAndWeight()方法时，必须有两个参数
...
> 可以把可变参数改写为String[]类型。但是，调用方需要自己先构造String[]，比较麻烦。调用方可以传入null。而可变参数可以保证无法传入null，因为传入0个参数时，接收到的实际值是一个空数组而不是null。
